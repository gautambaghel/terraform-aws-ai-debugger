...
from hcp_tf_utils import generate_ai_debugger_result
...
def eval(tf_plan_json):
    ...
    if guardrail_status:
        results.append(
            generate_ai_debugger_result(
                outcome_id="Plan-Summary",
                description="Summary of Terraform plan",
                result=description[:700],
            )
        )
    else:
        results.append(
            generate_ai_debugger_result(
                outcome_id="Plan-Summary",
                description="Summary of Terraform plan",
                result="Output omitted due to : {}".format(guardrail_response),
            )
        )
    ...
    if guardrail_status:
        results.append(
            generate_ai_debugger_result(
                outcome_id="AMI-Summary",
                description="Summary of AMI changes",
                result=result[:700],
            )
        )
    else:
...
def guardrail_inspection(input_text, input_mode="OUTPUT"):
    ...
    if guardrail_id and guardrail_version:
        logger.info(
            "##### Scanning Terraform plan output with Amazon Bedrock Guardrail #####"
        )
